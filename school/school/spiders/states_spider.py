...
class StateSpider(scrapy.Spider):
    ...
    start_urls = [
        "http://www.city-data.com/schools-dirs/schools-PA.html"
...
#	for sel in response.xpath('//div[@class="table-responsive"]/tbody/tr'): 
#		item = SchoolItem() 
...
#		yield item				
    ...
    def parse(self,response): 
        for sel in response.xpath('//table[@class="table"]/tbody/tr'):
	               ...
	               item['grade_span']=sel.xpath('td[5]/text()').extract() 
		       yield item
	               #print name,address,rating,enrollment,grade_span
